...
export default function PublicBooking() {
  ...
  if (loadingData) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:BG }}>
      <div style={{ color:'white', fontSize:18, textAlign:'center' }}><div style={{ fontSize:40, marginBottom:12 }}>✂️</div>Loading...</div>
    </div>
  );
    ...
    <div style={{ minHeight:'100vh', background:BG, paddingBottom:60 }}>
      ...
      <div style={{ padding:'24px 24px 0', maxWidth:680, margin:'0 auto' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:28 }}>
          <div style={{ width:44, height:44, borderRadius:12, background:'rgba(255,255,255,0.12)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>✂️</div>
          <div>
            <div style={{ color:'white', fontWeight:800, fontSize:20, fontFamily:'serif' }}>Salon</div>
      ...
      <div style={{ maxWidth:680, margin:'0 auto', padding:'0 16px' }}>
        <div style={card}>
          {result ? (
            ...
            <>
              <h2 style={{ fontSize:22, fontWeight:800, color:'#1F2937', marginBottom:4 }}>Book Your Visit</h2>
              ...
              <Section label="5. Select Services" error={errors.services}>
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {services.map(svc => {
                    ...
                    return (
                      <div key={svc.id} style={{ display:'flex', border:`2px solid ${sel?'#8B5CF6':'#E5E7EB'}`, borderRadius:12, overflow:'hidden', cursor:'pointer', background:sel?'#F5F3FF':'white', transition:'all 0.15s' }}>
                        {/* Thumbnail */}
                        <div style={{ width:72, height:72, flexShrink:0, background:'#F3F4F6', position:'relative', cursor:'pointer' }}
                          ...
                          {firstImg
                            ? (firstImg.file_type==='video'
                                ...
                                : <img src={MEDIA_URL(firstImg.file_name)} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />)
                          ...
                          >
                            <span style={{ color:'white', fontSize:14, opacity:0, transition:'opacity 0.2s' }}
                        ...
                        <div style={{ flex:1, padding:'10px 14px', display:'flex', alignItems:'center', justifyContent:'space-between' }}
                          ...
                          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                            <div style={{ width:22, height:22, borderRadius:'50%', flexShrink:0, border:`2px solid ${sel?'#8B5CF6':'#D1D5DB'}`, background:sel?'#8B5CF6':'white', display:'flex', alignItems:'center', justifyContent:'center' }}>
                              {sel && <span style={{ color:'white', fontSize:12 }}>✓</span>}
                            </div>
                            <div>
                              <div style={{ fontWeight:600, fontSize:14, color:'#1F2937' }}>{svc.name}</div>
                              <div style={{ fontSize:12, color:'#6B7280' }}>⏱ {svc.duration_minutes} min
                                {svc.media?.length>0 && <span style={{ marginLeft:6, color:'#8B5CF6' }}>📸 {svc.media.length}</span>}
                              </div>
                            </div>
                          </div>
                          <div style={{ fontWeight:800, fontSize:16, color:sel?'#8B5CF6':'#374151', flexShrink:0 }}>₹{parseFloat(svc.price).toLocaleString('en-IN')}</div>
                        </div>
                      </div>
                    );
                  })}
                ...
                {selectedServices.length > 1 && (
                  <div style={{ marginTop:8, fontSize:12, color:'#7C3AED', background:'#EDE9FE', padding:'6px 12px', borderRadius:8 }}>
                    ⏱ Total session: <strong>{totalDuration} minutes</strong>
                  </div>
                )}
                <div style={{ marginTop:6, fontSize:12, color:'#9CA3AF' }}>💡 Click the thumbnail to see service details</div>
              </Section>
